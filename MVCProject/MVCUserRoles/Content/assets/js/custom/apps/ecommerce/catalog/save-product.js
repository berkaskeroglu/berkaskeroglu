    ...
    return {
        init: function () {
            ...
            ["#kt_ecommerce_add_product_description", "#kt_ecommerce_add_product_meta_description"].forEach((e => {
                ...
                const tagifyElement = document.querySelector(e);

                if (tagifyElement) {
                    // AJAX isteği yaparak whitelist verilerini al
                    fetch(DZUrl2)
                        .then(response => {
                            if (!response.ok) {
                        ...
                        .then(data => {
                            ...
                            new Tagify(tagifyElement, {
                                whitelist: data, // AJAX isteği ile alınan whitelist verilerini kullan
                                dropdown: {
                                    maxItems: 20,
                ...
                init: function () {
                    ...
                    $("#kt_ecommerce_add_product_submit").click(function (e) {
                        
                        myDropzone.processQueue();
                        var formData = $('#formId').serializeArray();
                        $.ajax({
                            url: DZUrl, // Sunucu tarafındaki URL
                            method: "POST",
                            data: formData, // Form verileri
                            success: function (response) {
                                // İşlem başarılı olduğunda yapılacak işlemler
                                console.log("Form successfully submitted:", response);
                                // Sayfayı yeniden yükle, veya başka bir işlem yap
                            },  
